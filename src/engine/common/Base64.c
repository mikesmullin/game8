...
// char* encoded = base64_encode(g_engine->frameArena, (const unsigned char*)original_text, strlen(original_text));
// printf("Encoded: %s\n", encoded);

// Decode
// size_t decoded_len;
// unsigned char* decoded = base64_decode(g_engine->frameArena, encoded, &decoded_len);
